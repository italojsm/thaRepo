    ...
    <form id="multi" method="POST" action="controle/InsereHora.php">
        ...
        <tr>            
            ...
            <td>
             ...
             <td>
                <select class="tag" name="id_tarefa[]">
                  <?php
                       $stmt = $con->prepare("SELECT id_tarefa, observacao FROM tarefa;");
                       $stmt->execute();
                       while($aux = $stmt->fetch(PDO::FETCH_ASSOC)){?>
                  	        <option value="<?php echo $aux['id_tarefa']; ?>"><?php echo $aux['observacao']; ?></option>
                  
                 <?php } ?>
                  </select>
            </td>
            <td>
                <textarea rows="2" cols="50" name="observacao[]">
                </textarea>
            </td>
